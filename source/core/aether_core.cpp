...
void init(SystemState&s){
    ...
    quantum::init(q); engine::init(); graph::init(); recovery::init(); governor::init(); diag::init(); hil::init(); studio::init(); gate::init(); compute::init(); ui::init();
}
...
void update(SystemState&s){
    ...
    if(s.screen==0){
        ...
        if(d&KEY_TOUCH){
            ...
            else if(t.px<85) s.selectedModule=(s.selectedModule+MOD_COUNT-1)%MOD_COUNT;
            else if(t.px>170) s.selectedModule=(s.selectedModule+1)%MOD_COUNT;
            else doAction(s);
        }
