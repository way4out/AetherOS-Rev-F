...
void init(SystemState&s){
    s={false,false,false,false,false,false,false,false,0,0,0,false,false};

    // HARDWARE BOOT PATH: initialize video/UI before touching SD, Wi-Fi, audio,
    // benchmarks, or any service that can block on physical hardware.
    ...
    // Deliberately deferred: SD, benchmark, audio and network services.
    // The first frames must be rendered on a physical DSi before services start.
}
...
static void startDeferredServices(SystemState&s){
    ...
    // SD is safe to probe after the UI has been alive for several frames.
    ...
    // Do NOT auto-connect Wi-Fi during boot. The previous implementation could
    // block in Wifi_InitDefault(WFC_CONNECT) before the first frame was drawn.
    // Networking will remain a gateway service until explicitly initialized.
    radio::init();
    session::init();
    security::init();
...
void update(SystemState&s){
    ...
    // Give the DSi ~30 frames of guaranteed UI-only execution.
    if(s.frame==30)
        startDeferredServices(s);
    ...
    if(s.screen==0){
        ...
        if((d&KEY_TOUCH)&&t.px<256&&t.py<192){
            s.selectedModule=(t.py/24)%8;
            s.screen=s.selectedModule+1;
        }
    }else{
        if(d&KEY_A){
            if(s.screen==2) quantum::runBell(q);
            else if(s.screen==3){audio::tone(440,250);engine::saveProject();}
            else if(s.screen>=4&&s.screen<=6) engine::tick();
            else if(s.screen==8) engine::saveProject();
            s.projectSaved=engine::projectExists();
        }
        if(d&KEY_X&&s.screen==2) quantum::runGrover2(q);
        if(d&KEY_Y&&s.screen==2) quantum::measure(q);
        if(d&KEY_L&&s.screen==2) quantum::runDeutschJozsa(q);
        if(d&KEY_R&&s.screen==2) quantum::runQFT2(q);
