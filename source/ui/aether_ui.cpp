...
static void module(const SystemState&s){
    ...
    case MOD_MARAUDER:{auto sr=security::report();iprintf("AETHER MARAUDER / SECURITY LAB\\nMODE %s\\nCONSENT %s\\nTX LOCKED %s\\nCRED CAPTURE LOCKED %s\\n\\nPASSIVE RF: RSSI / CHANNEL / WATERFALL\\nAUTHORIZED NET: OWNED/LAB TRAFFIC METADATA\\nLAB SIM: SAFE ATTACK-CONCEPT SIMULATION\\nGATEWAY HARDEN: PROTOCOL / AUTH / CRC\\n\\nSAMPLES %lu DEVICES %lu PACKETS %lu\\nALERTS %lu LAB RUNS %lu\\n\\n%s",security::modeName(sr.mode),sr.consent?"YES":"REQUIRED",sr.txLocked?"YES":"NO",sr.credentialCaptureLocked?"YES":"NO",(unsigned long)sr.samples,(unsigned long)sr.devices,(unsigned long)sr.packets,(unsigned long)sr.alerts,(unsigned long)sr.labRuns,security::warning());break;}
