...
static void module(const SystemState&s){
    ...
    case MOD_NETWORK:{auto hs=heritage::state(); iprintf("NETWORK FABRIC\nOEQL %s\nSTELLARPHONE %s\nWIFI %s\n5G EXTERNAL GATEWAY\nSAT EXTERNAL GATEWAY\nBT EXTERNAL GATEWAY\nSDR EXTERNAL GATEWAY\nQPU EXTERNAL GATEWAY\nMARTIN OPTICS %s\nAUTHORIZED HARDWARE ONLY",hs.oeql?"READY":"OFF",hs.stellarphone?"READY":"OFF",s.networkReady?"LINK":"OFF",hs.martinOptics?"READY":"OFF");break;}
    ...
    case MOD_ANIMAL:{auto ar=animal::report(); iprintf("AETHER UNIVERSAL COMMUNICATION"); iprintf(" ANIMAL < > HUMAN BRIDGE"); iprintf(" SPECIES %s",animal::speciesName()); iprintf(" MODE %s",animal::directionName()); iprintf(" CONF %u%% SIGNAL %u",ar.confidence/10,ar.signalScore); iprintf(" SAMPLES %lu",(unsigned long)ar.samples); iprintf(" LOCAL ANALYSIS READY"); iprintf(" MODEL GATEWAY %s",animal::gatewayRequired()?"REQUIRED":"READY"); iprintf(" WEB: AETHERLINK / CRITTER CHAT"); iprintf(" TRANSLATE > VERIFY > SYNTHESIZE"); break;}
    case MOD_CODEX:{iprintf("YHWH (YHYyH) CODEX\nBIBLIO / READ-ONLY CORE\nENTRIES %u\nINVARIANT %s\nSOURCE %s\nCOMMIT %.8s\nSTATUS %s\nCODEX TICK %lu",codex::count(),codex::REVOLUTION_INVARIANT?"LOCKED":"FAULT",codex::CANONICAL_REPO,codex::CANONICAL_COMMIT,codex::status(),(unsigned long)s.codexTicks);break;}
    case MOD_HARMONIC:{auto n=harmonic::node();iprintf("PRIME NUMERICAL HARMONIC NODE\nPRIME %u  HARM %u\nVOID %d\nDAMP %u  AMP %u\nOUT %lu mHz\nSTACK %lu\n%s",(unsigned)n.prime,(unsigned)n.harmonic,(int)n.voidVector,(unsigned)n.dampener,(unsigned)n.amplifier,(unsigned long)harmonic::outputMilliHz(),(unsigned long)harmonic::stackHash(),harmonic::status());break;}
    case MOD_SYSTEM:{auto hs=heritage::state(); auto hr=hil::report();auto dg=diag::report();auto mr=mission::report();iprintf("SYSTEM HEALTH / MISSION CONTROL\nREADY SCORE %u%%\nCORE %s  SD %s  CFG %s\nQ %s  AUD %s  DSP %s  LAB %s\nAI %s  NET %s  SEC %s  REC %s\nGATEWAY %s\nHIL %u%%  DIAG %u  FAULTS %u\nGRAPH %u  ONLINE %u\nTX/CREDS/DESTRUCTIVE LOCKED\nOEQL %s\nSTELLARPHONE %s\nMARTIN OPTICS %s",mr.score,mission::state(mr.boot),mission::state(mr.sd),mission::state(mr.config),mission::state(mr.quantum),mission::state(mr.audio),mission::state(mr.dsp),mission::state(mr.lab),mission::state(mr.ai),mission::state(mr.network),mission::state(mr.security),mission::state(mr.recovery),mission::state(mr.gateway),hr.score,dg.score,dg.faults,dg.graphTicks,dg.gatewayOnline,hs.oeql?"READY":"OFF",hs.stellarphone?"READY":"OFF",hs.martinOptics?"READY":"OFF");break;}
    }
    actionPanel(m);
}
